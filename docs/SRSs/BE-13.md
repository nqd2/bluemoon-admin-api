...
*   **Payload:**
    ```json
    {
      ...
      "usage": 100, // Optional: chỉ cần khi fee unit là KWh hoặc WaterCube
      "unitPrice": 5000 // Optional: giá đơn vị
    }
    ```
...
* [x] **Duplicate Prevention:** Ngăn chặn đóng 2 lần cho phí bắt buộc (Service type). Cho phép đóng nhiều lần với phí tự nguyện (Contribution type).
* [x] **Data Validation:** `totalAmount` phải > 0. `payerName` không được để trống. `feeId` phải tồn tại trong database.
* [x] **Recording:** Lưu đúng thông tin người đóng (`payerName`), người thu (`createdBy` từ req.user), và thời gian (`date`).
* [x] **Response:** Trả về `201 Created` kèm thông tin transaction vừa tạo.
