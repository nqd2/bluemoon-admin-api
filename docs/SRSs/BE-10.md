...
*   **Payload Structure:**
    ```json
    {
      ...
      "amount": 5000,
      "unit": "m2" // Enum: 'hộ', 'khẩu', 'm2', 'KWh', 'WaterCube'
    }
    ```
*   **Data Validation (Zod Schema):**
    ...
    *   `unit`: Enum `['hộ', 'khẩu', 'm2', 'KWh', 'WaterCube']`. Required.
...
* [x] **Validation:**
    *   Nếu `type` không hợp lệ -> Trả về `400 Bad Request` với message chi tiết.
    *   Nếu `amount` < 0 -> Trả về `400 Bad Request`.
    *   Nếu `title` < 5 ký tự -> Trả về `400 Bad Request`.
* [x] **Success Case:**
    *   Tạo thành công -> Trả về `201 Created` kèm object Fee vừa tạo với format `{ success: true, message: '...', data: {...} }`.
* [x] **Data Integrity:**
    *   Field `createdAt` được tự động gán thời gian hiện tại (Mongoose timestamps).
