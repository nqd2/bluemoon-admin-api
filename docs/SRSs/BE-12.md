...
*   **Authentication:** Yêu cầu `Bearer Token` (Admin hoặc Accountant).
*   **Payload:**
    ```json
    {
      "apartmentId": "apt_id_123",
      "feeId": "fee_id_456",
      "usage": 100  // Optional: chỉ cần khi unit là KWh hoặc WaterCube
    }
    ```
...
3.  **Calculation Formula:**
    ...
    *   **Case 2: Unit = 'khẩu' (per person)**
        ...
        *   *Lưu ý hiện tại:* Code hiện tại đếm tất cả members trong mảng. Trong tương lai có thể cần filter theo `residencyStatus` (chỉ đếm "Thường trú" hoặc "Tạm trú") nhưng hiện tại đang đếm tất cả để đơn giản hóa cho MVP.
    *   **Case 4: Unit = 'KWh' hoặc 'WaterCube' (tiện ích)**
        *   `total = Fee.amount * usage` (usage được cung cấp trong request body)
