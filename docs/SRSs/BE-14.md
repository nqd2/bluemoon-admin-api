...
* [x] **Full Coverage:** Phải liệt kê được **TẤT CẢ** các căn hộ trong hệ thống, không chỉ những hộ đã đóng.
* [x] **Accuracy:** Trạng thái PAID/UNPAID phải chính xác dựa trên bảng Transactions. Mỗi apartment được kiểm tra xem có transaction tương ứng không.
* [x] **Summary:** Tính được tổng số tiền đã thu (`totalCollected`) cho khoản phí đó bằng cách sum tất cả `totalAmount` của các transactions.
* [x] **Error Handling:** Nếu fee không tồn tại, trả về 404.
