...
* [ ] **Header Check:** Kiểm tra sự tồn tại của header `Authorization` bắt đầu bằng `Bearer`.
* [ ] **Token Verification:** Sử dụng `jwt.verify` để kiểm tra tính hợp lệ của token với `JWT_SECRET`.
* [ ] **User Attachment:** Nếu token đúng, thông tin user (từ DB, bỏ password) phải được gán vào `req.user` để các controller phía sau sử dụng.
* [ ] **Error Handling:**
